#!/usr/bin/env python3
"""2-word vocab model: ['a','gpu'], trained on an alternating a/gpu corpus.
Learns to alternate -> generates 'a gpu a gpu ...'. Exports tinyllm_2w.bin."""
import torch, struct, numpy as np, math
from torch.nn import functional as F
import torch.nn as nn

VOCAB_LIST=["a","gpu"]
VOCAB=len(VOCAB_LIST)                       # 2
...
torch.manual_seed(0)
# alternating corpus a,gpu,a,gpu,...
data=torch.tensor([i%2 for i in range(4000)],dtype=torch.long).cuda()
model=TinyGPT().cuda()
