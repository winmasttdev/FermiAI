#!/usr/bin/env python3
"""Rare-word test: vocab ['a','gpu'], corpus = 4000 'a' with exactly ONE 'gpu'.
Does a tiny model ever emit the word it saw only once?"""
...
torch.manual_seed(0)
data=[0]*4000          # all 'a'
data[2000]=1           # exactly ONE 'gpu' at index 2000
...
# rigorous check: greedy-generate 300 tokens from 'a', count 'gpu'
...
print(f"greedy 300 tokens -> 'gpu' count = {cnt}  (seq sample: {seq[:20]})")
